...
class Pipe:
    def __init__(self,scale_factor,move_speed):
        self.img_up=pg.transform.scale_by(pg.image.load('background/pipeup.png').convert_alpha(),scale_factor)
        self.img_down = pg.transform.scale_by(pg.image.load('background/pipedown.png').convert_alpha(), scale_factor)#sclae factor is used to make the image dimension fit into the window.
